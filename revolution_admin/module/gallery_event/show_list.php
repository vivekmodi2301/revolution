	...
	<?php
		...
		$event_gallery_data=fetchAll("select id,category_id,photo,status from event_gallery where category_id=$_POST[id]");
		$totalRow=count($event_gallery_data);
	?>
	<table class="table table-striped" id="photoList">
	<tr>
		<th colspan="<?php if($totalRow<8){ echo $totalRow;}else{ echo 8;} ?>" style="text-align:center;">Event Gallery Photo</th>
	</tr>
	<tr>
		<th colspan="<?php if($totalRow<8){ echo $totalRow;}else{ echo 8;} ?>">
			...
			<div style="text-align:right; width=50%; float:right;"><a href="index.php?mod=gallery_event&do=event_gallery_addedit">Add New</a></div>
		</th>
	...
	<tr>
		<?php 
		$i=1;
		foreach ($event_gallery_data as $event_gallery_data) {
			if($i<8)
			{
		?>
				<td class="rowOrNot"><img src="system/event_gallery/<?php echo $event_gallery_data['photo'];?>" style="text-align:center" height="100px" width="100px">
					<br><input name="statuscheckid[]" type="checkbox" class="statuscheck" value="<?php echo $event_gallery_data['id'];?>" <?php if($event_gallery_data['status'] === 'y') echo 'checked="checked"';?> disabled>
					<input name="deletecheckid[]" type="checkbox" class="deletecheck" value="<?php echo $event_gallery_data['id'];?>" hidden>
				</td>
				<?php $i++;
			}
			else
