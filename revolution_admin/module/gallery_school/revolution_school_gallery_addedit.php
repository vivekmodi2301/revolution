<?php
	if (isset($_POST['submit']))
	{
		...
		if ($_FILES['photo']['name']) 
		{
			...
			if ($allImageFile) 
			{
				...
				for($i=0;$i<$count;$i++)
				{
					...
					move_uploaded_file($_FILES['photo']['tmp_name'][$i],'system/school_gallery/'.$photo[$i]);
				}
		...
		if($success)
		{
			...
			{
				$_POST['photo']=$p;
				addupdate('school_gallery',$_POST);
				header("location:index.php?mod=gallery_school&do=school_gallery_addedit");
			}
		}
	}
?>
...
<form method="post" enctype="multipart/form-data">
	<table class="table table-striped">
		<tr>
			<th colspan="2" style="text-align:center;">School Gallery Form</th>
		</tr>
		<?php
			if (isset($_POST['photo'])) {
				$p=array();
				...
				?>
				<tr>
					<td colspan="2"><img src="system/school_gallery/<?php echo $p['photo'];?>" height="100px" width="100px"><input type="hidden" name="photo" value="<?php echo $_POST['photo'];?>">
					</td>
				</tr>
				<?php
			}
